...
export class ProblemReasoningService {
  ...
  static generateSelectionReason(
    ...
    try {
      ...
      if (
        primaryTag &&
        userPerformance.weakTags &&
        userPerformance.weakTags.includes(primaryTag.toLowerCase())
      ) {
        const tagAccuracy =
          userPerformance.tagAccuracy?.[primaryTag.toLowerCase()] || 0;
        return {
          ...
          details: {
            ...
            targetAccuracy: 0.8,
            totalProblems:
              userPerformance.tagAttempts?.[primaryTag.toLowerCase()] || 0,
          },
      ...
      if (
        primaryTag &&
        userPerformance.newTags &&
        userPerformance.newTags.includes(primaryTag.toLowerCase())
      ) {
        return {
      ...
      if (sessionContext.difficultyProgression && problem.difficulty) {
        ...
        if (
          problem.difficulty.toLowerCase() === targetDifficulty?.toLowerCase()
        ) {
          return {
            ...
            details: {
              ...
              progressionReason:
                sessionContext.progressionReason || "performance-based",
            },
      ...
      if (sessionContext.patternReinforcement && primaryTag) {
        return {
          ...
          fullText: `Reinforcing successful patterns with ${primaryTag} problems (${Math.round(
            (userPerformance.tagAccuracy?.[primaryTag.toLowerCase()] || 0) * 100
          )}% success rate).`,
        };
      }
      ...
      if (isNewProblem) {
        return {
          ...
          fullText: `New problem selected to expand your experience with ${problemTags.join(
            ", "
          )} concepts.`,
        };
      }
  ...
  static hasRecentFailures(problem) {
    ...
    try {
      ...
      const sortedAttempts = [...problem.attempts].sort(
        (a, b) =>
          new Date(b.AttemptDate || b.date) - new Date(a.AttemptDate || a.date)
      );
  ...
  static getLastSuccessDate(problem) {
    ...
    try {
      ...
      return successfulAttempts.length > 0
        ? successfulAttempts[0].AttemptDate || successfulAttempts[0].date
        : null;
    } catch (error) {
