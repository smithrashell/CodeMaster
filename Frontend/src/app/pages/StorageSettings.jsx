...
export const StorageSettings = () => {
  ...
  const OverviewTab = () => (
    <div>
      ...
      {storageHealth && (
        <Card mb="lg">
          <Title order={4} mb="md">Health Summary</Title>
          
          <Group spacing="xl" mb="lg">
            <div>
              <Text size="sm" color="dimmed">IndexedDB Status</Text>
              <Group spacing="xs">
            ...
            <div>
              <Text size="sm" color="dimmed">Chrome Storage Status</Text>
              <Group spacing="xs">
                ...
                <Badge 
                  color={storageHealth.chromeStorage.available ? 'green' : 'red'}
                  size="sm"
                >
          ...
          {storageHealth.indexedDB.quota && (
            <div>
              <Text size="sm" mb={4}>IndexedDB Usage</Text>
              <Progress
                value={storageHealth.indexedDB.usagePercentage * 100}
                color={storageHealth.indexedDB.usagePercentage > 0.9 ? 'red' : 
                       storageHealth.indexedDB.usagePercentage > 0.8 ? 'yellow' : 'blue'}
                size="lg"
                label={`${Math.round(storageHealth.indexedDB.usagePercentage * 100)}%`}
              />
            </div>
          )}
          ...
          {storageHealth.chromeStorage.quota && (
            <div style={{ marginTop: '16px' }}>
              <Text size="sm" mb={4}>Chrome Storage Usage</Text>
              <Progress
                value={storageHealth.chromeStorage.usagePercentage * 100}
                color={storageHealth.chromeStorage.usagePercentage > 0.9 ? 'red' : 
                       storageHealth.chromeStorage.usagePercentage > 0.8 ? 'yellow' : 'blue'}
                size="lg"
                label={`${Math.round(storageHealth.chromeStorage.usagePercentage * 100)}%`}
              />
            </div>
          )}
        </Card>
  ...
  const CleanupTab = () => (
    <div>
      <Title order={3} mb="lg">Storage Cleanup</Title>
          ...
          <Accordion>
            ...
            <Accordion.Item value="chrome">
              <Accordion.Control icon={<IconCloudUpload size={20} />}>
                Chrome Storage Cleanup ({cleanupRecommendations.chromeStorage?.actions?.length || 0} actions)
              </Accordion.Control>
              <Accordion.Panel>
                {cleanupRecommendations.chromeStorage?.actions?.map((action, index) => (
                  <Group position="apart" key={index} p="sm">
                    <div>
                      <Badge 
                        size="xs" 
      ...
      {/* Cleanup Settings */}
      <Card>
        <Title order={4} mb="md">Cleanup Settings</Title>
        <Group>
          <Switch
  ...
  const MigrationTab = () => (
    <div>
      <Title order={3} mb="lg">Data Migration</Title>

      {/* Migration History */}
      <Card mb="lg">
        <Title order={4} mb="md">Recent Migrations</Title>
        
        {migrationHistory.length === 0 ? (
          <Text color="dimmed" ta="center">No migrations found</Text>
        ) : (
          <Table>
            <thead>
              <tr>
              ...
              {migrationHistory.map((migration) => (
                <tr key={migration.id}>
                  ...
                  <td>
                    <Badge 
                      size="sm"
                      color={migration.state === 'completed' ? 'green' : 
                             migration.state === 'failed' ? 'red' : 'blue'}
                    >
                      {migration.state}
                    </Badge>
                  </td>
                  <td>{formatDuration(migration.duration)}</td>
                  <td>{migration.migratedItems || migration.restoredItems || 0}</td>
      ...
      {/* Migration Controls */}
      <Card>
        <Title order={4} mb="md">Migration Tools</Title>
        <Group>
          <Button 
            leftIcon={<IconUpload size={16} />}
            onClick={async () => {
              try {
                ...
                if (result.success) {
                  addNotification('success', 'Migration to Chrome Storage completed');
                } else {
          ...
          <Button 
            ...
            onClick={async () => {
              try {
                const result = await StorageMigrationService.migrateFromChrome();
                if (result.success) {
                  addNotification('success', 'Restore from Chrome Storage completed');
                } else {
  ...
  const SettingsTab = () => (
    <div>
      <Title order={3} mb="lg">Storage Configuration</Title>

      <Card mb="lg">
        <Title order={4} mb="md">Fallback Settings</Title>
        <Select
          label="Fallback Mode"
      ...
      <Card>
        <Title order={4} mb="md">Advanced Options</Title>
        <Group>
          <Switch
