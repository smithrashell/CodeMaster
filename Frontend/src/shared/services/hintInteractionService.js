...
export class HintInteractionService {
  ...
  static async saveHintInteraction(interactionData, sessionContext = {}) {
    try {
      ...
      const interactionId = `hint_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      const completeInteraction = {
        ...
        hintId: interactionData.hintId || `${interactionData.hintType}_${interactionData.primaryTag}_${Date.now()}`,
        timestamp: new Date().toISOString(),
        sessionId: sessionId,
        boxLevel: boxLevel,
        userAction: interactionData.action || interactionData.userAction || 'clicked',
        problemDifficulty: problemDifficulty,
        tagsCombination: interactionData.problemTags || interactionData.tagsCombination || [],
        ...
        sessionContext: {
          ...
          totalHints: sessionContext.totalHints || interactionData.sessionContext?.totalHints,
          hintPosition: interactionData.sessionContext?.hintPosition,
          expandedHintsCount: interactionData.sessionContext?.expandedHintsCount,
      ...
      if (processingTime > 10) {
        console.warn("⚠️ Hint interaction save took longer than 10ms:", processingTime.toFixed(2) + "ms");
      }
  ...
  static async getProblemAnalytics(problemId) {
    try {
      ...
      interactions.forEach(interaction => {
        ...
        const hintKey = `${interaction.hintType}-${interaction.primaryTag}${interaction.relatedTag ? '-' + interaction.relatedTag : ''}`;
      ...
      analytics.engagementRate = interactions.length > 0 ? expansions / interactions.length : 0;
  ...
  static async getSessionAnalytics(sessionId) {
    try {
      ...
      Object.values(byProblem).forEach(problemInteractions => {
        const expansions = problemInteractions.filter(i => i.userAction === 'expand').length;
        const engagement = problemInteractions.length > 0 ? expansions / problemInteractions.length : 0;
        totalEngagement += engagement;
      });
      
      analytics.averageEngagementRate = Object.keys(byProblem).length > 0 
  ...
  static async getSystemAnalytics(filters = {}) {
    try {
      ...
      if (filters.difficulty) {
        interactions = interactions.filter(i => i.problemDifficulty === filters.difficulty);
      }
      
      if (filters.hintType) {
        interactions = interactions.filter(i => i.hintType === filters.hintType);
      }
  ...
  static async cleanupOldData(daysToKeep = 90) {
    try {
      ...
      // eslint-disable-next-line no-console
      console.log(`🧹 Cleaned up ${deletedCount} old hint interactions (older than ${daysToKeep} days)`);
  ...
  static _calculateHintTypePopularity(interactions) {
    const popularity = {};
    interactions.forEach(interaction => {
      popularity[interaction.hintType] = (popularity[interaction.hintType] || 0) + 1;
    });
  ...
  static _calculateDifficultyBreakdown(interactions) {
    ...
    Object.keys(breakdown).forEach(diff => {
      breakdown[diff].engagementRate = 
        breakdown[diff].total > 0 ? breakdown[diff].expanded / breakdown[diff].total : 0;
    });
  ...
  static _generateInsights(interactions, effectiveness) {
    ...
    if (effectivenessArray.length > 0) {
      ...
      insights.push(`Most effective hints: ${mostEffective.hintType} for ${mostEffective.difficulty} problems (${(mostEffective.engagementRate * 100).toFixed(1)}% engagement)`);
    }
    ...
    if (recentInteractions.length > 0) {
      insights.push(`${recentInteractions.length} hint interactions in the past week`);
    }
