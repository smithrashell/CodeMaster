...
export class ProblemRelationshipService {
  ...
  static async getSimilarProblems(problemId, limit = 10) {
    try {
      ...
      const { buildRelationshipMap } = await import("../db/problem_relationships.js");
  ...
  static async getProblemMetadata(problemId, existingProblemData = null) {
    try {
      ...
      console.error(`❌ Error getting problem metadata for ${problemId}:`, error);
      return null;
    }
  }
   ...
   */
  static async analyzeProblemContext(currentProblemId, problemTags, analysisDepth = 5) {
    try {
      console.log(`🔍 Analyzing problem context for problem ${currentProblemId}`);
      
      // Get similar problems
      const similarProblems = await this.getSimilarProblems(currentProblemId, analysisDepth);
   ...
   */
  static calculateRelationshipBonuses(currentTags, tagAnalysis, similarProblems) {
  ...
  static calculateContextStrength(similarProblems) {
    ...
    const avgWeight = similarProblems.reduce((sum, p) => sum + p.weight, 0) / similarProblems.length;
