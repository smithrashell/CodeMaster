...
jest.mock('../tagServices', () => ({
  TagService: {
    getCurrentTier: jest.fn().mockResolvedValue({ focusTags: ['array', 'string'] }),
...
describe('Core Session Integration Tests', () => {
  ...
  describe('🔁 Full Session Lifecycle Integration', () => {
    it('should complete full session creation → execution → completion cycle', async () => {
      ...
      const sessionState = MockDataFactories.scenarios.intermediateUser().sessionSettings;
      ...
      mockScheduleService.getDailyReviewSchedule.mockResolvedValue(reviewProblems);
      mockProblemsDb.fetchAdditionalProblems.mockResolvedValue(newProblems);
      mockProblemsDb.fetchAllProblems.mockResolvedValue([]);
      mockTagService.getCurrentTier.mockResolvedValue({ focusTags: ['array', 'string'] });
      ...
      if (!latestSession) {
        console.log('❌ No session was created - this indicates an issue with session creation');
      ...
      const completionResult = await SessionService.checkAndCompleteSession(sessionId);
    ...
    it('should handle session resumption correctly', async () => {
      ...
      const partialAttempts = mockProblems.slice(0, 2).map(p => 
        MockDataFactories.createMockAttempt({ problemId: p.id })
      );
  ...
  describe('🎯 User Performance Scenarios', () => {
    it('should handle new user onboarding flow', async () => {
      ...
      const newUserState = MockDataFactories.scenarios.newUser().sessionSettings;
    ...
    it('should handle expert user advanced settings', async () => {
      ...
      const expertState = MockDataFactories.scenarios.expertUser().sessionSettings;
  ...
  describe('📊 Data Persistence and Integrity', () => {
    it('should maintain referential integrity across session operations', async () => {
      ...
      // Create session
      await StorageService.setSessionState('session_state', MockDataFactories.scenarios.intermediateUser().sessionSettings);
  ...
  describe('⚠️ Error Handling', () => {
    it('should handle missing session gracefully', async () => {
      const result = await SessionService.checkAndCompleteSession('non-existent-session');
    ...
    it('should handle empty problem pools gracefully', async () => {
      ...
      await StorageService.setSessionState('session_state', MockDataFactories.scenarios.newUser().sessionSettings);
    ...
    it('should handle database errors gracefully', async () => {
      ...
      dbHelper.openDB = jest.fn().mockRejectedValue(new Error('Database connection failed'));
