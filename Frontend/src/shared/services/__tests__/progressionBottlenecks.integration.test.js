...
} from '../../utils/escapeHatchUtils.js';
...
describe('Progression Bottleneck Integration Tests', () => {
  ...
  describe('🔄 Adaptive Threshold System Integration', () => {
    ...
    it('should update struggle history appropriately', () => {
      ...
      const updated = updateStruggleHistory(currentHistory, performanceData, 'mastery');
      ...
      const masteryData = [
        {
          ...
          lastAttemptDate: new Date(Date.now() - 20 * 24 * 60 * 60 * 1000).toISOString() // 20 days ago
        }
      ];
  ...
  describe('🏆 Tier Progression Time-Based Escape', () => {
    it('should allow tier advancement after 30+ days with 60%+ completion', async () => {
      ...
      const tierProgressData = {
        tierStartDate: new Date(Date.now() - 35 * 24 * 60 * 60 * 1000).toISOString(),
        lastProgressDate: new Date(Date.now() - 32 * 24 * 60 * 60 * 1000).toISOString(),
      ...
      TagService.getCurrentTier.mockResolvedValue({
        ...
        allTagsInCurrentTier: ['array', 'hash-table', 'string', 'stack', 'queue'],
  ...
  describe('🎯 End-to-End Bottleneck Prevention', () => {
    it('should prevent permanent stagnation through multiple fallback mechanisms', async () => {
      ...
      expect(sessionResult.sessionState.escapeHatches.sessionsAtCurrentDifficulty).toBeGreaterThan(10);
