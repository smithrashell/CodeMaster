...
describe("DataIntegrityCheckService", () => {
  ...
  beforeEach(() => {
    ...
    // Setup schema mocks
    DataIntegritySchemas.getStoreNames.mockReturnValue([
      "problems",
      "attempts",
      "sessions",
    ]);
  ...
  describe("Monitoring (with proper cleanup)", () => {
    ...
    it("should handle monitoring intervals correctly", async () => {
      const performIntegrityCheckSpy = jest
        .spyOn(DataIntegrityCheckService, "performIntegrityCheck")
  ...
  describe("Error Handling", () => {
    ...
    it("should handle referential integrity service errors", async () => {
      ReferentialIntegrityService.checkAllReferentialIntegrity.mockRejectedValue(
        new Error("Referential check failed")
      );
