...
export class DataReconstructionService {
  ...
  static async reconstructData(options = {}) {
    const {
      types = [this.RECONSTRUCTION_TYPES.TAG_MASTERY, this.RECONSTRUCTION_TYPES.PROBLEM_STATS],
    ...
    console.log(`🔄 Starting data reconstruction (${dryRun ? 'DRY RUN' : 'LIVE'})...`);
    ...
    try {
      ...
      for (const type of types) {
        ...
        try {
          ...
          result.overall.recordsReconstructed += typeResult.recordsReconstructed || 0;
        ...
        result.performanceMetrics.typeBreakdown[type] = typeEndTime - typeStartTime;
      }
      ...
      console.log(`🏁 Data reconstruction completed: ${result.overall.recordsReconstructed} records reconstructed`);
      return result;
  ...
  static async reconstructDataType(db, type, options = {}) {
    const { strategy, dryRun = false, preserveExisting = true, priority = 'medium' } = options;

    switch (type) {
      case this.RECONSTRUCTION_TYPES.TAG_MASTERY:
        return await this.reconstructTagMastery(db, { strategy, dryRun, preserveExisting });
      
      case this.RECONSTRUCTION_TYPES.PROBLEM_STATS:
        return await this.reconstructProblemStats(db, { strategy, dryRun, preserveExisting });
      
      case this.RECONSTRUCTION_TYPES.SESSION_ANALYTICS:
        return await this.reconstructSessionAnalytics(db, { strategy, dryRun, preserveExisting });
      
      case this.RECONSTRUCTION_TYPES.CORRUPTED_SESSIONS:
        return await this.reconstructCorruptedSessions(db, { strategy, dryRun, preserveExisting });
      
      case this.RECONSTRUCTION_TYPES.PATTERN_LADDERS:
        return await this.reconstructPatternLadders(db, { strategy, dryRun, preserveExisting });
      
      case this.RECONSTRUCTION_TYPES.MISSING_RELATIONSHIPS:
        return await this.reconstructMissingRelationships(db, { strategy, dryRun, preserveExisting });
  ...
  static async reconstructTagMastery(db, options = {}) {
    const { strategy = this.STRATEGIES.FROM_ATTEMPTS, dryRun = false, preserveExisting = true } = options;
    ...
    try {
      ...
      const existingMasteryMap = new Map(existingMastery.map(tm => [tm.tag, tm]));
      ...
      for (const attempt of attempts) {
        ...
        for (const tag of problem.Tags) {
          ...
          if (!stats.lastAttemptDate || attemptDate > new Date(stats.lastAttemptDate)) {
      ...
      for (const [tag, stats] of tagStats.entries()) {
        ...
        if (preserveExisting && existingMasteryMap.has(tag)) {
          ...
          if (existing.totalAttempts === stats.totalAttempts && 
        ...
        const daysSinceLast = stats.lastAttemptDate ? 
          (Date.now() - new Date(stats.lastAttemptDate)) / (1000 * 60 * 60 * 24) : 0;
        
        const decayScore = stats.totalAttempts > 0 ? 
          Math.max(0.1, (1 - successRate) * (daysSinceLast / 30)) : 1;
        ...
        if (stats.totalAttempts >= 8 && successRate >= 0.75 && successRate < 0.8) {
          masteryThreshold = 0.75; // Light struggle escape
        } else if (stats.totalAttempts >= 12 && successRate >= 0.7 && successRate < 0.8) {
      ...
      if (!dryRun && reconstructedMastery.length > 0) {
        ...
        console.log(`✅ Reconstructed ${reconstructedMastery.length} tag mastery records`);
      }
  ...
  static async reconstructProblemStats(db, options = {}) {
    ...
    try {
      ...
      for (const problem of problems) {
        ...
        const problemAttempts = attempts.filter(a => 
          a.ProblemID === problem.id || 
          a.problemId === problem.leetCodeID
        );
        ...
        const successfulAttempts = problemAttempts.filter(a => a.Success || a.success).length;
        ...
        if (preserveExisting && existingStats && 
        ...
        const updatedProblem = {
          ...
          lastAttemptDate: problemAttempts
            .sort((a, b) => new Date(b.AttemptDate || b.date) - new Date(a.AttemptDate || a.date))[0]
            ?.AttemptDate || problem.lastAttemptDate,
          reconstructed: true,
          reconstructionDate: new Date().toISOString()
        };
        ...
        if (lastAttempt) {
          const boxCalculation = await calculateLeitnerBox(updatedProblem, lastAttempt);
      ...
      if (!dryRun && updatedProblems.length > 0) {
        ...
        console.log(`✅ Reconstructed statistics for ${updatedProblems.length} problems`);
      }
  ...
  static async reconstructSessionAnalytics(db, options = {}) {
    ...
    try {
      ...
      const existingAnalyticsMap = new Map(existingAnalytics.map(sa => [sa.sessionId, sa]));
      ...
      for (const session of sessions) {
        ...
        // Calculate analytics
        const totalProblems = session.problems ? session.problems.length : sessionAttempts.length;
        const totalAttempts = sessionAttempts.length;
        const successfulAttempts = sessionAttempts.filter(a => a.Success || a.success).length;
        const accuracy = totalAttempts > 0 ? successfulAttempts / totalAttempts : 0;

        const totalTime = sessionAttempts.reduce((sum, a) => sum + (a.TimeSpent || a.timeSpent || 0), 0);
        ...
        const tagsWorkedOn = [...new Set(
          session.problems?.flatMap(p => p.Tags || []) || []
        )];

        // Calculate efficiency score (accuracy vs average time)
        const expectedTime = this.getExpectedTimeByDifficulty(predominantDifficulty);
        const efficiencyScore = expectedTime > 0 ? 
          Math.min(1, expectedTime / averageTimePerProblem) * accuracy : accuracy;
        ...
        const analytics = {
          ...
          improvementAreas: this.calculateImprovementAreas(accuracy, averageTimePerProblem, predominantDifficulty),
          reconstructed: true,
          reconstructionDate: new Date().toISOString()
        };
      ...
      if (!dryRun && analyticsToReconstruct.length > 0) {
        ...
        console.log(`✅ Reconstructed analytics for ${analyticsToReconstruct.length} sessions`);
      }
  ...
  static async reconstructCorruptedSessions(db, options = {}) {
    ...
    try {
      ...
      const orphanedAttempts = attempts.filter(a => !a.SessionID || a.SessionID === 'unknown');
      ...
      for (const [dateKey, dateAttempts] of sessionGroups.entries()) {
        ...
        const sessionId = `reconstructed_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        
        // Get unique problems for this session
        const problemIds = [...new Set(dateAttempts.map(a => a.ProblemID || a.problemId))];
        const sessionProblems = problems.filter(p => 
          problemIds.includes(p.id) || problemIds.includes(p.leetCodeID)
        );
      ...
      if (!dryRun && reconstructedSessions.length > 0) {
        ...
        for (const session of reconstructedSessions) {
          for (const sessionAttempt of session.attempts) {
            const attempt = orphanedAttempts.find(a => a.id === sessionAttempt.attemptId);
            if (attempt) {
        ...
        console.log(`✅ Reconstructed ${reconstructedSessions.length} sessions from orphaned attempts`);
      }
  ...
  static async reconstructPatternLadders(db, options = {}) {
    ...
    try {
      ...
      const existingLaddersMap = new Map(existingLadders.map(pl => [pl.tag, pl]));
      ...
      for (const mastery of tagMastery) {
        ...
        const tagProblems = standardProblems.filter(p => 
          p.tags && p.tags.includes(mastery.tag)
        );
        ...
        result.details.push({
          ...
          difficulties: {
            ...
            Medium: ladderProblems.filter(p => p.difficulty === 'Medium').length,
      ...
      if (!dryRun && laddersToReconstruct.length > 0) {
        ...
        console.log(`✅ Reconstructed ${laddersToReconstruct.length} pattern ladders`);
      }
  ...
  static async reconstructMissingRelationships(db, options = {}) {
    ...
    try {
      ...
      let relationshipId = Math.max(...existingRelationships.map(r => r.id), 0) + 1;
      ...
      for (const [tag, problems] of tagGroups.entries()) {
        ...
        for (let i = 0; i < problems.length; i++) {
          for (let j = i + 1; j < problems.length; j++) {
            ...
            const sharedTags = problem1.tags.filter(t => problem2.tags.includes(t));
            const strength = sharedTags.length / Math.max(problem1.tags.length, problem2.tags.length);
            ...
            if (strength >= 0.3) {
              const relationship = {
                ...
                relationshipType: this.determineRelationshipType(problem1, problem2, sharedTags),
      ...
      if (!dryRun && newRelationships.length > 0) {
        ...
        console.log(`✅ Created ${newRelationships.length} problem relationships`);
      }
  ...
  static generateReconstructionId() {
    return `reconstruction_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  }
  ...
  static addToReconstructionHistory(result) {
    ...
    if (this.reconstructionHistory.length > this.maxHistorySize) {
      this.reconstructionHistory = this.reconstructionHistory.slice(-this.maxHistorySize);
    }
  }
  ...
  static async reportReconstructionError(operation, error, context) {
    try {
      await ErrorReportService.storeErrorReport({
        errorId: `reconstruction_error_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
  ...
  static getReconstructionStatistics() {
    ...
    const successfulReconstructions = this.reconstructionHistory.filter(r => r.overall.success).length;
    const totalRecordsReconstructed = this.reconstructionHistory.reduce((sum, r) => sum + r.overall.recordsReconstructed, 0);

    return {
      totalReconstructions,
      successfulReconstructions,
      totalRecordsReconstructed,
      successRate: totalReconstructions > 0 ? (successfulReconstructions / totalReconstructions) * 100 : 0,
      averageRecordsPerReconstruction: totalReconstructions > 0 ? totalRecordsReconstructed / totalReconstructions : 0
    };
  }
  ...
  static async validateReconstructedData(type) {
    ...
    try {
      ...
      switch (type) {
        ...
        default:
          return { success: false, error: `Validation not implemented for type: ${type}` };
      }
    } catch (error) {
  ...
  static async validateTagMasteryIntegrity(db) {
    ...
    for (const mastery of tagMastery.filter(tm => tm.reconstructed)) {
      ...
      if (mastery.mastered && mastery.successfulAttempts === 0) {
        issues.push(`Tag ${mastery.tag}: marked as mastered with no successful attempts`);
      }
    }
