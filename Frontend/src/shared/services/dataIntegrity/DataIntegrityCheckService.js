...
export class DataIntegrityCheckService {
  ...
  static async performIntegrityCheck(options = {}) {
    ...
    console.log(`🔍 Starting ${checkType} data integrity check for ${stores.length} stores...`);
    ...
    try {
      // Schema validation
      if ([this.CHECK_TYPES.FULL, this.CHECK_TYPES.SCHEMA, this.CHECK_TYPES.QUICK].includes(checkType)) {
      ...
      if ([this.CHECK_TYPES.FULL, this.CHECK_TYPES.REFERENTIAL].includes(checkType)) {
      ...
      if ([this.CHECK_TYPES.FULL, this.CHECK_TYPES.BUSINESS_LOGIC].includes(checkType)) {
        const businessStartTime = performance.now();
        console.log('🧠 Running business logic validation...');
        
        report.results.businessLogic = await this.performBusinessLogicValidation(stores, { priority });
      ...
      console.log(`✅ Data integrity check completed in ${report.performanceMetrics.totalTime.toFixed(2)}ms`);
      console.log(`📊 Overall Score: ${report.overall.score}% (${report.overall.errors} errors, ${report.overall.warnings} warnings)`);
  ...
  static async performSchemaValidation(stores, options = {}) {
    ...
    for (const storeName of stores) {
      ...
      try {
        ...
        console.log(`✅ ${storeName}: ${storeResult.recordCount} records, ${storeResult.errors.length} errors, ${storeResult.warnings.length} warnings`);

      } catch (error) {
        console.error(`❌ Schema validation failed for store ${storeName}:`, error);
        result.valid = false;
    ...
    result.performanceMetrics.totalTime = Object.values(result.performanceMetrics.storeBreakdown)
      .reduce((sum, time) => sum + time, 0);
    result.performanceMetrics.avgRecordTime = result.totalRecords > 0 
  ...
  static async checkCrossStoreConsistency(result) {
    ...
    try {
      ...
      for (const problem of problems) {
        if (problemsWithAttempts.has(problem.leetCodeID)) {
          const problemAttempts = attempts.filter(a => a.problemId === problem.leetCodeID);
          const actualTotal = problemAttempts.length;
          const actualSuccessful = problemAttempts.filter(a => a.success).length;
            ...
            if (TotalAttempts !== actualTotal || SuccessfulAttempts !== actualSuccessful) {
              ...
              result.checks.push({
                ...
                details: {
                  ...
                  calculated: { TotalAttempts: actualTotal, SuccessfulAttempts: actualSuccessful }
                }
              });
            }
          }
      ...
      for (const session of sessions) {
        if (session.attempts) {
          for (const sessionAttempt of session.attempts) {
            const attemptExists = attempts.some(a => a.id === sessionAttempt.attemptId);
            if (!attemptExists) {
              result.valid = false;
              result.warningCount++;
              result.checks.push({
    ...
    const endTime = performance.now();
    result.performanceMetrics.checkBreakdown.crossStoreConsistency = endTime - startTime;
  }
  ...
  static async checkDataFreshness(result, stores) {
      ...
      if (stores.includes('sessions')) {
        ...
        if (staleSessions.length > 0) {
          ...
          result.checks.push({
            ...
            details: { count: staleSessions.length, sessions: staleSessions.map(s => s.id) }
          });
        }
      }
      ...
      if (stores.includes('tag_mastery')) {
        ...
        if (outdatedMastery.length > 0) {
          ...
          result.checks.push({
            ...
            details: { count: outdatedMastery.length, tags: outdatedMastery.map(tm => tm.tag) }
          });
        }
      }
    ...
    const endTime = performance.now();
    result.performanceMetrics.checkBreakdown.dataFreshness = endTime - startTime;
  }
  ...
  static async checkStatisticalAnomalies(result, stores) {
    ...
    try {
      ...
      if (stores.includes('tag_mastery')) {
        ...
        const suspiciousHighRates = tagMastery.filter(tm => 
          tm.successRate >= 1.0 && 
          tm.totalAttempts > 0 && 
          tm.totalAttempts < 3
        );
        ...
        if (suspiciousHighRates.length > 0) {
          ...
          result.checks.push({
            ...
            details: { 
              ...
              tags: suspiciousHighRates.map(tm => ({ tag: tm.tag, attempts: tm.totalAttempts }))
            }
          });
        }
      }
      ...
      if (stores.includes('attempts')) {
        ...
        if (suspiciouslyFast.length > 5) { // Only flag if there are many
          result.warningCount++;
          result.checks.push({
        ...
        const suspiciouslySlow = recentAttempts.filter(a => a.timeSpent > 7200);
    ...
    const endTime = performance.now();
    result.performanceMetrics.checkBreakdown.statisticalAnomalies = endTime - startTime;
  }
  ...
  static calculateOverallScore(report) {
    ...
    if (report.results.schema) {
      const schemaScore = report.results.schema.totalRecords > 0 
        ? (report.results.schema.validRecords / report.results.schema.totalRecords) * 100
        : 100;
      score += schemaScore;
      factors++;
    }
    ...
    if (report.results.businessLogic) {
      const businessScore = Math.max(0, 100 - (report.results.businessLogic.errorCount * 10) - (report.results.businessLogic.warningCount * 5));
      score += businessScore;
      factors++;
    }
    ...
    if (report.results.storageHealth) {
      ...
      else if (report.results.storageHealth.overall === 'warning') healthScore = 60;
      else if (report.results.storageHealth.overall === 'good') healthScore = 90;
      score += healthScore;
      factors++;
    }
  ...
  static generateRecommendations(report) {
    ...
    if (report.results.referential && !report.results.referential.overall.valid) {
    ...
    if (report.performanceMetrics.totalTime > 10000) { // More than 10 seconds
      recommendations.push({
        type: 'performance',
        priority: 'medium',
    ...
    if (report.results.storageHealth && 
      ...
      recommendations.push({
        type: 'storage_health',
        priority: report.results.storageHealth.overall === 'critical' ? 'high' : 'medium',
  ...
  static startPeriodicMonitoring(config = {}) {
    ...
    if (!this.monitoringIntervals.has('quick')) {
      const quickInterval = setInterval(async () => {
        try {
          ...
          if (!result.overall.valid && result.overall.errors > 0) {
            console.warn(`⚠️ Quick check found ${result.overall.errors} errors`);
      ...
      console.log(`✅ Quick checks scheduled every ${quickCheckInterval / 1000 / 60} minutes`);
    }
    ...
    if (!this.monitoringIntervals.has('full')) {
      const fullInterval = setInterval(async () => {
        try {
          ...
          if (autoRepair && result.results.referential?.repairSuggestions) {
            const safeRepairs = result.results.referential.repairSuggestions
              .filter(s => s.automated && s.risk === 'low');
            
            if (safeRepairs.length > 0) {
              console.log(`🔧 Auto-repairing ${safeRepairs.length} low-risk issues...`);
      ...
      console.log(`✅ Full checks scheduled every ${fullCheckInterval / 1000 / 60 / 60} hours`);
    }
  ...
  static async getIntegrityDashboardSummary() {
    const recentCheck = this.checkHistory.length > 0 
  ...
  static calculateTrends() {
    ...
    const recentAvg = recent.reduce((sum, check) => sum + check.overall.score, 0) / recent.length;
    const olderAvg = older.reduce((sum, check) => sum + check.overall.score, 0) / older.length;
  ...
  static calculateAverageScore() {
    ...
    const sum = this.checkHistory.reduce((sum, check) => sum + check.overall.score, 0);
    return Math.round(sum / this.checkHistory.length);
  }

  static getChecksToday() {
    const today = new Date().toDateString();
    return this.checkHistory.filter(check => 
      new Date(check.timestamp).toDateString() === today
    ).length;
  }
