...
import { ReasonTypeIcon } from "./ui/ProblemInfoIcon";
 ...
 */
const WhyThisProblem = ({ selectionReason, problemTags = [], className = "" }) => {
  ...
  const handleToggle = () => {
    ...
    if (!isExpanded) {
      console.log('📊 Why This Problem section opened for reason:', selectionReason.type);
    }
  };
    ...
    switch (type) {
        ...
        return (
          <div style={{ fontSize: '13px', color: 'rgba(255, 255, 255, 0.8)', lineHeight: '1.4' }}>
            <div style={{ marginBottom: '6px' }}>
              📊 <strong>Current Performance:</strong> {Math.round((details.currentAccuracy || 0) * 100)}% accuracy
            </div>
            <div style={{ marginBottom: '6px' }}>
              🎯 <strong>Target Performance:</strong> {Math.round((details.targetAccuracy || 0.8) * 100)}% accuracy
            </div>
            {details.totalProblems > 0 && (
              <div>
                📈 <strong>Problems Attempted:</strong> {details.totalProblems} with {details.weakTag} tag
              </div>
            )}
          </div>
        );
      ...
      case 'spaced_repetition':
        return (
          <div style={{ fontSize: '13px', color: 'rgba(255, 255, 255, 0.8)', lineHeight: '1.4' }}>
            <div style={{ marginBottom: '6px' }}>
              ⏰ <strong>Last Attempt:</strong> {details.daysSinceLastAttempt} days ago
            </div>
            <div style={{ marginBottom: '6px' }}>
              🎯 <strong>Optimal Review Interval:</strong> {details.optimalInterval} days
            </div>
            {details.previousAttempts > 0 && (
              <div>
                📚 <strong>Previous Attempts:</strong> {details.previousAttempts} time(s)
              </div>
            )}
          </div>
        );
        ...
        return (
          <div style={{ fontSize: '13px', color: 'rgba(255, 255, 255, 0.8)', lineHeight: '1.4' }}>
            ...
            {details.totalNewTags > 1 && (
              <div>
                📖 <strong>Total New Tags in Session:</strong> {details.totalNewTags}
              </div>
            )}
          </div>
        );
      ...
      case 'difficulty_progression':
        return (
          <div style={{ fontSize: '13px', color: 'rgba(255, 255, 255, 0.8)', lineHeight: '1.4' }}>
            ...
            <div>
              🎯 <strong>Progression Reason:</strong> {details.progressionReason === 'performance-based' ? 'Based on your recent performance' : 'Systematic advancement'}
            </div>
          </div>
        );
      ...
      case 'performance_recovery':
        return (
          <div style={{ fontSize: '13px', color: 'rgba(255, 255, 255, 0.8)', lineHeight: '1.4' }}>
            <div style={{ marginBottom: '6px' }}>
              💪 <strong>Recent Struggles:</strong> {details.recentFailures} unsuccessful attempts
            </div>
            {details.lastSuccess && (
              <div>
                ✅ <strong>Last Success:</strong> {new Date(details.lastSuccess).toLocaleDateString()}
              </div>
            )}
          </div>
        );
      ...
      case 'pattern_reinforcement':
        return (
          <div style={{ fontSize: '13px', color: 'rgba(255, 255, 255, 0.8)', lineHeight: '1.4' }}>
            ...
            <div>
              📊 <strong>Current Success Rate:</strong> {Math.round((details.successRate || 0) * 100)}%
            </div>
          </div>
        );
        ...
        return (
          <div style={{ fontSize: '13px', color: 'rgba(255, 255, 255, 0.8)', lineHeight: '1.4' }}>
            ...
            {details.problemTags && details.problemTags.length > 0 && (
              <div>
                🏷️ <strong>Algorithm Tags:</strong> {details.problemTags.join(', ')}
              </div>
            )}
          </div>
        );
      ...
      case 'review_problem':
        return (
          <div style={{ fontSize: '13px', color: 'rgba(255, 255, 255, 0.8)', lineHeight: '1.4' }}>
            <div style={{ marginBottom: '6px' }}>
              📚 <strong>Review Session:</strong> {details.totalAttempts} previous attempt(s)
            </div>
            {details.daysSinceLastAttempt > 0 && (
              <div>
                ⏰ <strong>Last Seen:</strong> {details.daysSinceLastAttempt} days ago
              </div>
            )}
          </div>
        );
      ...
      default:
        return (
          <div style={{ fontSize: '13px', color: 'rgba(255, 255, 255, 0.8)' }}>
            🤖 Selected by adaptive learning algorithm based on your progress patterns.
          </div>
        );
    }
  };
  ...
  const getReasonIcon = (type) => {
    switch (type) {
      case 'tag_weakness':
        return { component: <span style={{ fontSize: '16px' }}>⚠️</span>, color: '#f59e0b' };
      case 'spaced_repetition':
        return { component: <span style={{ fontSize: '16px' }}>🔄</span>, color: '#3b82f6' };
      case 'new_tag_introduction':
        return { component: <span style={{ fontSize: '16px' }}>✨</span>, color: '#10b981' };
      case 'difficulty_progression':
        return { component: <span style={{ fontSize: '16px' }}>📈</span>, color: '#8b5cf6' };
      case 'performance_recovery':
        return { component: <span style={{ fontSize: '16px' }}>💪</span>, color: '#ef4444' };
      case 'pattern_reinforcement':
        return { component: <span style={{ fontSize: '16px' }}>🎯</span>, color: '#06b6d4' };
      case 'review_problem':
        return { component: <span style={{ fontSize: '16px' }}>📚</span>, color: '#6b7280' };
      case 'new_problem':
        return { component: <span style={{ fontSize: '16px' }}>🆕</span>, color: '#10b981' };
      default:
        return { component: <BrainIcon className="problem-sidebar-section-icon" />, color: 'var(--cd-link)' };
    }
  };
      ...
      {isExpanded && (
        <div className="problem-sidebar-primer-content" style={{ 
          ...
          <div style={{ 
            ...
            {/* Detailed breakdown */}
            {selectionReason.details && renderReasonDetails(selectionReason.type, selectionReason.details)}
            ...
            <div style={{ 
              ...
              💡 <strong>Learning Tip:</strong> {getLearningTip(selectionReason.type)}
            </div>
          </div>
        </div>
      )}
