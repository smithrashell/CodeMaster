...
const SmartPopover = ({ 
  ...
  const [position, setPosition] = useState({ x: 0, y: 0, placement: 'bottom-start' });
  ...
  useEffect(() => {
    ...
    const calculatePosition = () => {
      ...
      const optimalWidth = Math.min(width, Math.max(280, spaceRight - 20, spaceLeft - 20));
      setActualWidth(optimalWidth);

      // Determine vertical placement (bottom vs top)
      const requiredHeight = Math.min(maxHeight, 350);
      const useBottom = spaceBelow >= requiredHeight + 20 || (spaceBelow > spaceAbove && spaceBelow >= 200);
        ...
        <div
          style={{
            ...
            ...(position.placement.startsWith('bottom') ? {
              ...
              borderTopWidth: 0,
              left: position.placement.endsWith('start') ? 16 : actualWidth - 28,
            } : {
              bottom: -12,
        ...
        <div
          style={{
            ...
            ...(position.placement.startsWith('bottom') ? {
              ...
              borderTopWidth: 0,
              left: position.placement.endsWith('start') ? 15 : actualWidth - 30,
            } : {
              bottom: -14,
