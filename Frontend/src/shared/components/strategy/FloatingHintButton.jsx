...
const FloatingHintButton = ({ problemTags = [], onOpen, onClose }) => {
  ...
  const generalHints = hints.filter(hint => hint.type === 'general' || hint.type === 'pattern');
  ...
  return (
    ...
    >
      <Popover.Target>
        <Tooltip label={`${totalHints} strategy hints available`} position="top">
          <button
            ref={buttonRef}
            onClick={() => {
        ...
        <div style={{ padding: '16px' }}>
          ...
          <Group justify="space-between" mb="sm">
            <Group gap="xs">
              ...
              <Text fw={600} size="sm">Strategy Hints</Text>
            </Group>
            <Group gap="xs">
              {problemTags.map(tag => (
                ...
                {generalHints.map((hint, index) => (
                  <Card key={`general-${index}`} p="sm" radius="sm" bg="gray.0">
                    <Group gap="xs" mb="xs">
                      ...
                      {hint.relatedTag && (
                        <>
                          <Text size="xs" c="dimmed">pattern:</Text>
