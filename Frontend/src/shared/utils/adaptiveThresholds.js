...
function adaptDifficultyThresholds(adaptations, history) {
  const {
    ...
    consecutiveFailedPromotions: _consecutiveFailedPromotions = 0,
  } = history;
...
function adaptExpansionThresholds(adaptations, history) {
  const {
    sessionsAtSameTagCount = 0,
    expansionFailures: _expansionFailures = 0,
  } = history;
 ...
 */
export function updateStruggleHistory(
  currentHistory = {},
  performanceData,
  context
) {
...
function updateDifficultyStruggleHistory(history, data) {
  ...
  if (promoted) {
    ...
    history.consecutiveFailedPromotions =
      (history.consecutiveFailedPromotions || 0) + 1;
  }
...
function updateMasteryStruggleHistory(history, data) {
  ...
  if (history.lastMasteryDate) {
    ...
    history.daysWithoutProgress = Math.floor(
      (new Date() - lastMastery) / (1000 * 60 * 60 * 24)
    );
  }
