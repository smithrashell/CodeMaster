...
describe("Escape Hatch Utilities", () => {
  ...
  describe("calculateAdjustedThreshold", () => {
    it("should return reduced difficulty threshold when session-based escape hatch is active", () => {
      ...
      const threshold = calculateAdjustedThreshold(
        escapeHatchResults,
        "difficulty"
      );
    ...
    it("should return reduced mastery threshold for attempt-based escape hatch", () => {
      const escapeHatchResults = {
        ...
        attemptBased: [{ tag: "dynamic-programming", adjustedThreshold: 0.6 }],
        timeBased: [],
      };
    ...
    it("should return default thresholds when no escape hatches are active", () => {
      ...
      expect(calculateAdjustedThreshold(escapeHatchResults, "difficulty")).toBe(
        0.9
      );
      expect(
        calculateAdjustedThreshold(escapeHatchResults, "mastery", "array")
      ).toBe(0.8);
    });
  });
  ...
  describe("updateEscapeHatchTracking", () => {
    it("should track newly activated escape hatches", () => {
      ...
      const updatedState = updateEscapeHatchTracking(
        sessionState,
        escapeHatchResults
      );

      expect(updatedState.escapeHatches.activatedEscapeHatches).toContain(
        "session-based"
      );
      expect(updatedState.escapeHatches.activatedEscapeHatches).toContain(
        "attempt-based-graph"
    ...
    it("should not duplicate escape hatch tracking entries", () => {
      ...
      const activatedCount =
        updatedState.escapeHatches.activatedEscapeHatches.filter(
          (h) => h === "session-based"
        ).length;
      ...
      const escapeHatchResults = {
        recommendations: [
          ...
          },
          {
            type: "attempt-based",
            message:
              "Attempt-based escape hatch activated for dynamic-programming",
