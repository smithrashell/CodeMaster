 ...
 */
export function detectApplicableEscapeHatches(sessionState, masteryData, tierTags) {
  const now = new Date();
  ...
  if (results.timeBased.length > 0) {
    results.timeBased.forEach(tag => {
      const daysSinceLastAttempt = Math.floor((now - new Date(tag.lastAttemptDate)) / (1000 * 60 * 60 * 24));
      results.recommendations.push({
        type: 'time-based',
...
function detectSessionBasedEscapeHatch(escapeHatches) {
  const sessionsAtCurrentDifficulty = escapeHatches.sessionsAtCurrentDifficulty || 0;
 ...
 */
export function calculateAdjustedThreshold(escapeHatchResults, type, tag = null) {
  if (type === 'difficulty') {
  ...
  if (type === 'mastery' && tag) {
    ...
    const attemptBasedTag = escapeHatchResults.attemptBased.find(t => t.tag === tag);
    if (attemptBasedTag) {
      return attemptBasedTag.adjustedThreshold;
    }
    
    // Check time-based escape hatch for this tag
    const timeBasedTag = escapeHatchResults.timeBased.find(t => t.tag === tag);
    if (timeBasedTag) {
...
export function updateEscapeHatchTracking(sessionState, escapeHatchResults) {
  ...
  if (escapeHatchResults.sessionBased.applicable && !activatedHatches.includes('session-based')) {
