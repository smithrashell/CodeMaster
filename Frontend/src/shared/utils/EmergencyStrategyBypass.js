...
const EMERGENCY_STRATEGIES = {
  array: {
    tag: "array",
    strategy:
      "Use two pointers for sorted arrays, sliding window for subarrays, binary search for searches. Consider prefix sums for range queries.",
    overview:
      "Arrays store elements in contiguous memory. Key patterns: iteration, two pointers, sliding window, binary search.",
  ...
  "hash table": {
    tag: "hash table",
    strategy:
      "Use HashMap for O(1) lookups, count frequencies, detect duplicates. Great for caching and memoization.",
    overview:
      "Hash tables provide O(1) average-case lookup, insertion, and deletion through key-value mapping.",
    patterns: [
      "Frequency Count",
      "Lookup Table",
      "Memoization",
  ...
  string: {
    tag: "string",
    strategy:
      "Use two pointers for palindromes, sliding window for substrings, KMP for pattern matching, trie for multiple strings.",
    overview:
      "Strings are arrays of characters. Key operations: substring, palindrome, pattern matching, manipulation.",
    patterns: [
      "Two Pointers",
      "Sliding Window",
      "Pattern Matching",
  ...
  sorting: {
    tag: "sorting",
    strategy:
      "Sort first to enable binary search and two pointers. Use merge sort for stability, quick sort for average performance.",
    overview:
      "Sorting arranges elements in order. Enables binary search, merging, and simplifies many problems.",
  ...
  tree: {
    tag: "tree",
    strategy:
      "Use DFS for path problems, BFS for level-order traversal. Consider tree properties: BST ordering, balanced trees.",
    overview:
      "Trees are hierarchical data structures with parent-child relationships. Root, leaves, depth, height concepts.",
  ...
  "dynamic programming": {
    ...
    strategy:
      "Identify optimal substructure and overlapping subproblems. Use memoization (top-down) or tabulation (bottom-up).",
    overview:
  ...
  "binary search": {
    tag: "binary search",
    strategy:
      "Requires sorted data. Use for search, find boundaries, or search in answer space. Template: left, right, mid.",
    overview:
  ...
  "two pointers": {
    tag: "two pointers",
    strategy:
      "Use left/right pointers for sorted arrays, fast/slow for linked lists, sliding window for subarrays.",
    overview:
      "Two pointers technique uses two indices to traverse data structure efficiently.",
  ...
  "linked list": {
    tag: "linked list",
    strategy:
      "Use dummy nodes to simplify edge cases, two pointers for cycles, reverse in groups using iteration or recursion.",
    overview:
      "Linked lists store data in nodes with pointers. Dynamic size but no random access.",
  ...
  stack: {
    tag: "stack",
    strategy:
      "LIFO structure. Use for parentheses matching, expression evaluation, monotonic stack for next greater element.",
    overview:
      "Stack follows Last-In-First-Out principle. Useful for backtracking, parsing, and maintaining order.",
...
export class EmergencyStrategyBypass {
  ...
  enableEmergencyMode() {
    ...
    console.warn(
      "🚨 EMERGENCY MODE: Strategy system bypassed - using fallback strategies only"
    );
  }
  ...
  disableEmergencyMode() {
    this.isEmergencyMode = false;
    console.log(
      "✅ Emergency mode disabled - normal strategy retrieval restored"
    );
  }
  ...
  getEmergencyStrategy(tag) {
    ...
    if (strategy) {
      this.fallbackCount++;
      console.log(
        `🔄 EMERGENCY: Using fallback strategy for "${tag}" (${this.fallbackCount} total fallbacks)`
      );
      return strategy;
    }
  ...
  generateBasicStrategy(tag) {
    console.log(
      `⚠️ EMERGENCY: Generating basic strategy for unknown tag "${tag}"`
    );
...
if (typeof window !== "undefined") {
  window.enableEmergencyStrategy = () => {
    ...
    console.log(
      "🚨 Emergency strategy mode enabled. Extension will use fallback data only."
    );
  };
  ...
  window.disableEmergencyStrategy = () => {
    ...
    console.log(
      "✅ Emergency strategy mode disabled. Extension will try normal operation."
    );
  };
